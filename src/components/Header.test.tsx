...
describe('Header', () => {
  it('navigates from links and invokes the theme control', () => {
    ...
    fireEvent.click(document.querySelector('#nav-portfolio')!);
    fireEvent.click(screen.getByRole('button', { name: 'Toggle Theme' }));

    expect(props.onNavigate).toHaveBeenCalledWith('portfolio', 'none');
    expect(props.toggleTheme).toHaveBeenCalledOnce();
  });

  it('routes to portfolio when a search begins and keeps the query', () => {
    const props = renderHeader();
